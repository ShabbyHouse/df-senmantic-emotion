...
for j, (train_in, test_in) in enumerate(kf.split(X, y)):
    ...
    print('sub.oof_sub before:', oof_sub.shape)
    oof_sub = oof_sub + clf.predict_proba(sub)
    print('sub.oof_sub:', oof_sub.shape)
...
print('finish')
result[['id', 'label']].to_csv('./baseline_lr_tfidf_{}.csv'.format(str(np.mean(xx_cv)).split('.')[1]), index=False)
